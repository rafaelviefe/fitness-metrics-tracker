...
[x] ID: 130: Update `WeightStatisticsCard.tsx` to conditionally use `formatDateWithTimeForDisplay` if `displayTime` prop is true and a record is present, otherwise use `formatDateForDisplay`.