...
[x] ID: 009: Implement a test in `src/app/page.test.tsx` to verify that the `Home` component correctly loads and applies the `sortOrder` preference from `localStorage` on initial render, ensuring the correct sort toggle is active if set.
